...
export class AlertHeartbeatService implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly workerName = `alert@${hostname()}:${process.pid}`;
  private readonly workerQueues = 'alerts, notifications';
  ...
  async onModuleDestroy() {
    ...
    try {
      const redisClient = await this.alertEvaluationQueue.client;
