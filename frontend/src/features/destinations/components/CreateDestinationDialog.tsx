...
import {
  createDestination,
  getDestinationTypesClient,
} from '../api/destinationsClientApi';
import type { DestinationTypeDefinition } from '../types';
import {
...
export default function CreateDestinationDialog({
  onClose,
}: CreateDestinationDialogProps) {
  const router = useRouter();
  const [types, setTypes] = useState<DestinationTypeDefinition[]>([]);
  const [searchText, setSearchText] = useState('');
  const [selectedType, setSelectedType] =
    useState<DestinationTypeDefinition | null>(null);
  ...
  useEffect(() => {
    ...
    void getDestinationTypesClient()
      .then((nextTypes) => {
        if (isMounted) {
          setTypes(nextTypes);
        }
        ...
        if (isMounted) {
          setError(
            nextError instanceof Error
              ? nextError.message
              : 'Failed to load alert destination types.',
          );
        }
      })
      .finally(() => {
  ...
  const formId = selectedType
    ? `create-destination-${selectedType.type}`
    : 'create-destination';
          ...
          {!selectedType ? (
            <div>
              ...
              <div className="mt-4 divide-y divide-slate-200 rounded-sm border border-slate-200">
                {isLoading ? (
                  ...
                  <div className="px-5 py-8 text-center text-[14px] text-slate-500">
                    Alert destination 유형을 불러오지 못했습니다.
                  </div>
            ...
            <div>
              ...
              <DynamicDataSourceForm
                ...
                fields={buildDestinationFields(selectedType)}
                formId={formId}
                hideSubmitButton
