...
import CreateDataSourceDialog from '@/features/data-sources/components/CreateDataSourceDialog';
import { getDataSourceLogoPath } from '@/features/data-sources/utils/logo';
import type { SettingsDataSourceItem } from '../types';
...
export default function DataSourcesSection({
  ...
  const [isModalOpen, setIsModalOpen] = useState(openOnLoad && canManage);
  ...
  const openCreateModal = () => {
    ...
    if (pathname !== '/data_sources/new') {
      router.push('/data_sources/new');
    }
  };
    ...
    <div>
      ...
      {items.length === 0 ? (
        <div className="rounded-sm border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center text-[14px] text-slate-500">
          <div>There are no data sources yet.</div>
          {canManage ? (
            <div className="mt-2">
              <button
      ...
      {canManage && isModalOpen ? (
        <CreateDataSourceDialog onClose={closeCreateModal} />
      ) : null}
    </div>
  );
}
