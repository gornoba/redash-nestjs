...
async function proxyUsersRequest(
  ...
  const response = await fetch(
    `${getApiBaseUrl()}/api/users/${path.join('/')}${url.search}`,
    {
      method,
      headers: {
...
export async function GET(
  ...
  return proxyUsersRequest(request, 'GET', path);
}
...
export async function POST(
  ...
  return proxyUsersRequest(request, 'POST', path);
}
...
export async function DELETE(
  ...
  return proxyUsersRequest(request, 'DELETE', path);
}
